...
## Overview
This project shows how the Elastic Stack (ELK) can be used for practical log analysis in a SOC environment.
We will walk through Kibana’s core features, run KQL searches, analyze VPN activity, and build visualizations and a simple monitoring dashboard.

---

## 1. Objective

- Get familiar with the main Elastic Stack components (Elasticsearch, Logstash, Beats, Kibana)  
- Use Kibana’s Discover, Visualize, and Dashboard pages  
- Practice filtering and investigating logs with KQL  
- Review VPN logs for suspicious patterns or anomalies  
- Create basic visualizations and build a VPN activity dashboard   
...
### Elasticsearch  
Stores JSON log data and supports fast search and analytics.

### Logstash  
Handles data ingestion and parsing (input → filter → output) before sending logs to Elasticsearch.
...
4. Kibana provides tools to analyze and visualize  
...
KQL is used throughout Kibana for filtering and investigating logs.
